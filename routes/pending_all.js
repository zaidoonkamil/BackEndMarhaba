...
router.get("/pending-all",async (req, res) => {
    try {
      const [pendingAdresses, pendingAnothers, pendingHalls, pendingFarms, pendingTourism] = await Promise.all([
        ...
        Tourism.findAll({ where: { status: 'pending' } }),
      ]);
      ...
      const pendingItems = [
        ...
        ...pendingTourism.map(item => ({ ...item.dataValues, type: 'tourism' })),
      ];
